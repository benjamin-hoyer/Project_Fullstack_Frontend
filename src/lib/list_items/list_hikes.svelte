<script lang="ts">
	import type { Category, Hike } from '../../services/types.ts';
	...
	<div class="columns is-multiline is-3>">
		{#each hikes as hike}
			<div class="column is-one-third">
				<div class="card">
					...
					<div class="card-content">
						<div class="media">
							<div class="media-content">
								<p>
									<a href="category/{hike.categoryid}/hike/{hike._id}" class="title is-3"
										>{hike.name}</a
									>
								</p>
					...
					<div class="card">
						<footer class="card-footer">
							<a href="category/{hike.categoryid}/hike/{hike._id}" class="card-footer-item"
								>Look up</a
							>
						</footer>
					</div>
				...
				<div>
					<LeafletMap
						markerLayers={[getMarkerLayer([hike]), getMarkerLayerEnd([hike])]}
						id={hike._id}
						height={20}
						location={{ lat: hike.lat, lng: hike.long }}
						zoom={8}
					/>
				</div>
			</div>
		...
		{#if addHike === true && category._id !== ''}
			<div
				...
				style="display:flex; border-style: dashed; border-radius: 30px"
			>
				<a
					class="button is-rounded"
					style="font-size: 4em;  margin: auto;"
					href="category/{category._id}/addHike"
				>
