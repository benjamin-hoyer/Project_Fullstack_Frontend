<script lang="ts">
	...
	export function populateLayer(markerLayer: MarkerLayer) {
		...
		markerLayer.markerSpecs.forEach((markerSpec) => {
			...
			if (markerSpec.popup) {
				const imgString =
					'<figure class="image is4by3"><img src=\'' +
					markerSpec.img +
					"' width='100%' alt='not found'/></figure>";
				const link =
					"<a class='subtitle is-6' href='category/" +
					markerSpec.categoryid +
					'/hike/' +
					markerSpec.id +
					"'>" +
					markerSpec.title +
			...
			// eslint-disable-next-line @typescript-eslint/ban-ts-comment
			// @ts-ignore
			else polyLineEnd.push(markerSpec.location);
		});
...
<div {id} class="box" style="height: {height}vh; z-index: 0" />
