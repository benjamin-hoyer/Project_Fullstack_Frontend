...
<div class:is-active="{current}" class="dropdown "  on:click={()=> current = !current} style="margin-top: 20px; ">
    ...
    <div class="dropdown-menu" id="dropdown-menu" role="menu">
        <div class="dropdown-content">
            <p class="dropdown-item has-text-weight-bold" on:click={()=> ( filteredPublicHikeStore.set(get(publicHikeStore)))} >
               All public Hikes
            </p>
            {#each categories as category}
                    <hr class="dropdown-divider">
                    <p class="dropdown-item" on:click={filter(category)}>
                        {category.name}
                    </p>
            {/each}
